...
export function CalendarTab({ language, t }: CalendarTabProps) {
  ...
  return (
    <div className="space-y-4 pb-24">
      ...
      {/* AI Chat Box */}
      <Card className="bg-card backdrop-blur-md border-border overflow-hidden relative">
        <div className="absolute inset-0 pointer-events-none z-0">
        ...
        <div className="bg-[#d92228] p-4 relative z-10 flex items-center justify-between">
          <h3 className="text-primary-foreground">{t.aiNutritionAssistant}</h3>
          <span className="text-primary-foreground text-xs">{language === 'en' ? 'Uses left: ' : 'Lượt còn lại: '} {Math.max(0, 20 - usageCount)}</span>
        </div>

        {/* Chọn chế độ và nút chụp ảnh (chỉ hiển thị khi chọn Phân tích calories) */}
        <div className="p-4 border-b border-gray-300 relative z-10 flex items-center gap-2">
        ...
        <ScrollArea className="h-64 p-4 relative z-10">
          <div className="space-y-3">
        ...
        {/* Khung camera chụp ảnh cho chế độ calories */}
        {showCamera && (
        ...
        <div className="p-4 border-t border-gray-300 relative z-10">
          <div className="flex gap-2">
            <Input
              value={inputMessage}
            ...
            <Button
              ...
              disabled={isSending}
              className="bg-[#d92228] hover:opacity-90"
            >
              <Send size={20} />
            </Button>
          </div>
